...
import { useSetSelectedContactId, useShowFavouritesOnly, useSearch } from '../../hooks/useZustandSelectors';
...
const ContactList = ({ navigate, page, rowsPerPage, setTotalCount }) => {
  ...
  const handleFavouriteToggle = useCallback((e, contact) => {
    e.stopPropagation();
    updateContactMutation.mutate({ id: contact.id, ...contact, favourite: !contact.favourite });
  }, [updateContactMutation]);

  return (
    <Box sx={{ width: '100%', bgcolor: 'softCardBg.main', display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', bgcolor: 'softCardBg.main', justifyContent: 'space-between', mb: 2, px: 2, pt: 2 }}>
